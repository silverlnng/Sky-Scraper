...
public class VRRightHandle : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, OVRInput.Controller.RTouch) && catchObj == null)
        {
            CatchObject(); //총을 잡는다
        }
        ...
        else if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, OVRInput.Controller.RTouch) && catchObj != null)
        {
            DropObject(); //총을 놓는다
        }
        ...
        {
            catchObj.transform.position = Vector3.Lerp(catchObj.transform.position, RightHand.position, catchSpeed * Time.deltaTime);
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(RightHand.position, catchRadius);

    }

    void CatchObject()
    {
        Ray ray = new Ray(RightHand.position, RightHand.forward); //레이를 생성 ( 위치 , 방향)
        ...
        if (hitInfos != null && hitInfos.Length > 0) //부딪힌 경우가 있는 경우 실행
        {
            ...
            hitInfos[0].transform.parent = RightHand; // 부딪힌것을 Hand의 자식으로 ==부딪힌것의 부모를 Hand 로 설정

            //hitInfos[0].transform.position = RightHand.position;
            //hitInfos[0].transform.forward = RightHand.forward;

            /*  hitInfos[0].transform.rotation = RightHand.rotation; *///손의 회전각도와 총의 회전각도 맞춰줌 
